...
def load_data_cart(filename: str) -> (np.recarray,)*3:
	with fits.open(filename) as catalog_data:
		catalog_data = catalog_data[1].data
		x = catalog_data['x']
		y = catalog_data['y']
...
def load_data_cart_weighted(filename: str) -> (np.recarray,)*4:
	with fits.open(filename) as catalog_data:
		catalog_data = catalog_data[1].data
		x = catalog_data['x']
		y = catalog_data['y']
		z = catalog_data['z']
		if catalog_data.columns[3].name == 'wts':
			weight = catalog_data['wts']
		else:
			catalog_data.columns.change_name(catalog_data.columns[3].name,'wts')
			weight = catalog_data['wts']
	return x, y, z, weight


def save_data(filename_new: str, ra: np.array, dec: np.array, z: np.array):
	...
	t.writeto(filename_new, overwrite=True)
    
def save_data_cart(filename_new: str, x: np.array, y: np.array, z: np.array):
	xcol = fits.Column(name='x', array=ra, format='E')
	ycol = fits.Column(name='y', array=dec, format='E')
	zcol = fits.Column(name='z', array=z, format='E')
	t = fits.BinTableHDU.from_columns([xcol, ycol, zcol])
	t.writeto(filename_new, overwrite=True)


def save_data_weighted(filename_new: str, ra: np.array, dec: np.array, z: np.array, w: np.array):
...
def save_data_weighted_cart(filename_new: str, x: np.array, y: np.array, z: np.array, w: np.array):
	# 'E' is the fits format for single-precision floating point
	xcol = fits.Column(name='x', array=x, format='E')
	ycol = fits.Column(name='y', array=y, format='E')
	zcol = fits.Column(name='z', array=z, format='E')
	wcol = fits.Column(name='wts', array=w, format='E')
	t = fits.BinTableHDU.from_columns([xcol, ycol, zcol, wcol])
	t.writeto(filename_new, overwrite=True)
...
def cart_input(x: np.array, y: np.array, z: np.array) -> (np.array, np.array, np.array):
	xs = x
	ys = y
	zs = z
	return xs, ys, zs
...
def cartesian2cartesian(xs: np.array, ys: np.array, zs: np.array) -> (np.array,)*4:
	xs = xs
	ys = ys
	zs = zs
	return xs, ys, zs
