...
class CenterFinder():
	...
	def __init__(self, galaxy_file: str, wtd: bool,  cart: bool, params_file: str, save: bool, printout: bool,
		...
		self.filename_new = self.filename.replace('/','_')
		self.save = save
		self.savename = f'out_{self.filename_new}/'
		self.printout = printout
		if not cart:
			self.cart_check = False
		else:
			self.cart_check = True 
		...
		if not cart:         
			...
			if not wtd:
				self.G_x, self.G_y, self.G_z = load_data_cart(galaxy_file)
				self.G_weights = np.ones(len(self.G_x), dtype=float)
			else:
				self.G_x, self.G_y, self.G_z, self.G_weights = load_data_cart_weighted(galaxy_file)
	...
	def __str__(self):
		if self.cart_check == True:
			return 'CenterFinder object\n'\
					...
					f'X range: [{self.G_x.min()}, {self.G_x.max()}]\n'\
					f'Y range: [{self.G_y.min()}, {self.G_y.max()}]\n'\
					f'Z range: [{self.G_z.min()}, {self.G_z.max()}]'
		else:
			return 'CenterFinder object\n'\
					f'Galaxy data file: {self.filename}\n'\
					f'Kernel radius: {self.kernel_radius}\n'\
	...
	def _vote(self, cart: bool, dencon: tuple = (False, False), overden: bool = False, 
		...
		if not cart:
			...
			xyzs = cart_input(self.G_x, self.G_y, self.G_z)

		self.galaxies_cartesian = np.array(xyzs).T  # each galaxy is represented by (x, y, z)

		# gets the 3d histogram (density_grid) and the grid bin coordinates in cartesian (grid_edges)
		...
		if self.save:
			np.save(self.savename + f'density_grid_r_{self.kernel_radius}_no_cut.npy', density_grid)
	...
	def find_centers(self, cart: bool, dencon: bool, overden: bool):
		"""
		...
		self._vote(cart, dencon=dencon, overden=overden)
		...
		if cart:
			...
			self.C_x, self.C_y, self.C_z = cartesian2cartesian(*C_xyzs)
			...
			save_data_weighted_cart(savename, self.C_x, self.C_y, self.C_z, self.C_weights)
		else:
			C_xyzs = np.array([centers_bin_coords[i][self.centers_indices[i]] for i in range(len(self.centers_indices))])
			self.C_ra, self.C_dec, self.C_redshift, _ = cartesian2sky(*C_xyzs, self.LUT_redshifts, self.G_ra.min(), self.G_ra.max())
			# outputs centers catalog in skycoords+weights to out folder
			savename = self.savename + f'found_centers_r_{self.kernel_radius}_cut_{self.vote_threshold}.fits'
